...
from pyhawkes.internals.weights import (GammaMixtureWeights,
...
class _DiscreteTimeNetworkHawkesModelBase(object, metaclass=abc.ABCMeta):
    ...
    def initialize_with_standard_model(self, standard_model=None):
        ...
        if standard_model is None:
            ...
            # standard_model = DiscreteTimeStandardHawkesModel(
            #     K=self.K, dt=self.dt, dt_max=self.dt_max, B=self.B,
            #     basis=self.basis, allow_self_connections=False)
    ...
    def add_data(self, S, F=None, minibatchsize=None, indices=None):
        """
        Add a data set to the list of observations.
        First, filter the data with the impulse response basis,
        then instantiate a set of parents for this data set.
        ...
        if minibatchsize is not None:
            for offset in np.arange(T, step=minibatchsize):
                ...
                S_mb = S[offset:end, :]
                F_mb = F[offset:end, :]
                minibatchfrac = float(T_mb) / T

                # Instantiate parent object for this minibatch
                parents = self._parent_class(self, T_mb, S_mb, F_mb, minibatchfrac=minibatchfrac)
                # Add minibatch to the data list
                self.data_list.append(parents)
                self.data_idx.append((offset+indices[0], end+indices[0]))
                # print "Added MB (%s,%s)" % ((offset+indices[0],end+indices[0]))
        else:
    ...
    def expected_log_likelihood(self, indices=None):
        exp_ll = 0
        ...
        for ind in indices:
            exp_ll += self.data_list[ind].expected_log_likelihood()
...
class DiscreteTimeNetworkHawkesModelGammaMixture(
    ...
    def sgd_step(self, minibatchsize, stepsize):
        ...
        if not hasattr(self, 'sgd_offset'):
            ...
            self.sgd_offset += 1
            if self.sgd_offset >= len(self.data_list):
                self.sgd_offset = 0
        ...
        # Grab one dataset from the data_list and assume
        # it has been added in minibatches
        p = self.data_list[self.sgd_offset]
        # S, F, T, minibatchfrac = p.S, p.F, p.T, p.minibatchfrac
        minibatchfrac = p.minibatchfrac
...
class ContinuousTimeNetworkHawkesModel(ModelGibbsSampling):
    ...
    _default_exp_impulse_hypers = {"alpha_0": 1.0, "beta_0": 1.0}
    _default_weight_hypers = {"parallel_resampling": True}
    ...
    @property
    def Amask(self):
        return self.weight_model.Amask
    ...
    @property
    def parents(self):
        return [copy.deepcopy(_.Z) for _ in self.data_list]
    ...
    def compute_rate_at_events(self, data):
        ...
        min_supp, max_supp = self.impulse_model.support
        from pyhawkes.internals.continuous_time_helpers import compute_rate_at_events
        lmbda = np.zeros(N)
        compute_rate_at_events(S, C, dt_max, min_supp, max_supp, lambda0, W, mu, tau, lmbda)
    ...
    def resample_model_excluding_parents(self):
        # Update the bias model given the parents assigned to the background
        self.bias_model.resample(self.data_list)
        # Update the impulse model given the parents assignments
        self.impulse_model.resample(self.data_list)
