...
class Basis(object, metaclass=abc.ABCMeta):
    ...
    def convolve_with_basis(self, S):
        ...
        # # Compute convolutions in parallel
        from joblib import Parallel, delayed
        args = iter(np.reshape(self.basis[:, b], (R, 1)) for b in np.arange(B))
        for b, c_b in zip(np.arange(B), Parallel(n_jobs=-1)(delayed(sig.fftconvolve)(in1=S, in2=arg, mode='full') for arg in args)):
            F[:,:, b] = c_b[:T,:]

        # Compute convolutions of each basis vector, one at a time
        # for b in np.arange(B):
        #     F[:,:,b] = sig.fftconvolve(S, np.reshape(self.basis[:,b],(R,1)), 'full')[:T,:]
...
class CosineBasis(Basis):
    ...
    def create_basis(self):
        ...
        ctrs = u_ir[np.floor(np.linspace(n_eye, (n_pts/2.0), n_cos)).astype(np.int)] # loading front half - Andy
        if len(ctrs) == 1:
            w = ctrs/2
        else:
