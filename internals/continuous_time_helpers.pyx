...
cdef inline double translate_dt(double dt, double dt_max, double min_supp, double max_supp) nogil:
    """
    Translate dt in range (0,dt_max) into corresponding dt in range (min_supp,max_supp)
    """
    return dt_max * (((dt/dt_max) * (max_supp-min_supp)) + min_supp)
...
cpdef ct_resample_Z_logistic_normal_serial(
    double[::1] S, long[::1] C, long[::1] Z, double dt_max, double min_supp, double max_supp,
    ...
    for n in range(N):
        ...
        for par in range(n-1, -1, -1):
            ...
            dt_translated = translate_dt(dt,dt_max,min_supp,max_supp)
            p[par] = W[C[par], C[n]] * ln_impulse(dt_translated, mu[C[par], C[n]], tau[C[par], C[n]], dt_max)
            denom += p[par]
...
cpdef ct_Z_post_distr_logistic_normal(
    double[::1] S, long[::1] C, double[:,::1] p_prob,
    double dt_max, double min_supp, double max_supp,
    double[::1] lambda0, double[:,::1] W, double[:,::1] mu, double[:,::1] tau):
    ...
    for n in prange(N, nogil=True):
        ...
        for par in range(n-1, -1, -1):
            ...
            dt_translated = translate_dt(dt,dt_max,min_supp,max_supp)
            p_prob[n,par+1] = W[C[par], C[n]] * ln_impulse(dt_translated, mu[C[par], C[n]], tau[C[par], C[n]], dt_max)
...
cpdef ct_resample_Z_logistic_normal(
    ...
    for n in prange(N, nogil=True):
        ...
        for par in range(n-1, -1, -1):
            ...
            dt_translated = translate_dt(dt,dt_max,min_supp,max_supp)
            p = W[C[par], C[n]] * ln_impulse(dt_translated, mu[C[par], C[n]], tau[C[par], C[n]], dt_max)
            denom = denom + p
...
cpdef ct_compute_suff_stats(
    ...
    for n in range(N):
        ...
        if par == -1:
            ...
            dt = translate_dt(S[n] - S[par],dt_max,min_supp,max_supp)
            imp_ss[0, C[par], C[n]] += 1
            imp_ss[1, C[par], C[n]] += np.log(dt) - np.log(np.maximum(1e-8,dt_max - dt))
    ...
    for n in range(N):
        ...
        if par > -1:
            dt = translate_dt(S[n] - S[par],dt_max,min_supp,max_supp)
            sdt = np.log(dt) - np.log(np.maximum(1e-8,dt_max - dt))
...
cpdef compute_rate_at_events(
    double[::1] S, long[::1] C, double dt_max, double min_supp, double max_supp,
    ...
    for n in prange(N, nogil=True):
        ...
        for par in range(n-1, -1, -1):
            ...
            if W[C[par], C[n]] > 0:
                dt_translated = translate_dt(dt,dt_max,min_supp,max_supp)
                lmbda[n] += W[C[par], C[n]] * ln_impulse(dt_translated, mu[C[par], C[n]], tau[C[par], C[n]], dt_max)
...
cpdef compute_weighted_impulses_at_events(
    ...
    for n in prange(N, nogil=True):
        ...
        for par in range(n-1, -1, -1):
            ...
            dt_translated = translate_dt(dt,dt_max,min_supp,max_supp)
            lmbda[n, cp] += W[cp, cn] * ln_impulse(dt_translated, mu[cp, cn], tau[cp, cn], dt_max)
