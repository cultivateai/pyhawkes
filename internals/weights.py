...
def ct_resample_column_of_A_fast(args):
    P_k, K, Ns, T, W_k, lambda0_k, A_mask, A_col, lambda_ir_k = args

    u_k = np.random.rand(K)
    resample_A_col(P_k, K, A_col, u_k, A_mask, W_k,
                   lambda0_k, lambda_ir_k, Ns, T)
...
class GammaMixtureWeights(GibbsSampling, MeanField, MeanFieldSVI):
    ...
    def __init__(self, model, kappa_0=0.1, nu_0=10.0, Amask=None):
        ...
        assert (Amask.shape == (self.K, self.K))
        self.Amask = Amask.astype(bool).astype(np.float32)
        self.A = np.ones((self.K, self.K)) * self.Amask
        self.W = np.zeros((self.K, self.K))
        ...
        self.mf_p = np.clip(self.Amask * np.ones((self.K, self.K)), 1e-8, 1-1e-8)
    ...
    def expected_log_W(self):
        ...
        if not self.network.allow_self_connections:
            np.fill_diagonal(E_ln_W, np.log(1e-8))
    ...
    def resample_from_mf(self):
        ...
        :return:
        """
        self.A = self.Amask * (np.random.rand(self.K, self.K) < self.mf_p)
    ...
    def _resample_A(self, ss):
        ...
        #           = lp1 - ln(exp(lp0) + exp(lp1))
        #           = lp1 - Z
        rawA = np.log(np.random.rand(self.K, self.K)) < lp1 - Z
        self.A = rawA * self.Amask
    ...
    def initialize_from_gibbs(self, A, W, scale=100):
        ...
        # and (1-conf) if A == 0
        conf = 0.95
        mf_p_raw = conf * A + (1-conf) * (1-A)
        self.mf_p = self.Amask * mf_p_raw
...
class SpikeAndSlabContinuousTimeGammaWeights(GibbsSampling):
    ...
    def __init__(self, model, parallel_resampling=True, Amask=None):
        ...
        self.Amask = Amask.astype(bool).astype(np.float32)
        self.A = np.ones((self.K, self.K))
        self.W = np.clip(np.zeros((self.K, self.K)), 1e-16, np.inf)
        self.resample()
    ...
    @property
    def W_effective(self):
        return self.A * self.Amask * self.W
    ...
    def _compute_weighted_impulses_at_events_manual(self, data):
        ...
        for n in range(N):
            ...
            for par in range(n-1, -1, -1):
                ...
                lmbda[n, C[par]] += W[C[par], C[n]] * self.model.impulse_model.impulse(dt, C[par], C[n])
    ...
    def _compute_weighted_impulses_at_events(self, data):
        ...
        lmbda = np.zeros((N, self.K))
        compute_weighted_impulses_at_events(S, C, Z, dt_max, min_supp, max_supp, W, mu, tau, lmbda)
    ...
    def _resample_A_given_W(self, data):
        ...
        # Compute weighted impulse response manually
        # lmbda_irs = [self._compute_weighted_impulses_at_events_manual(d) for d in data]
    ...
    def _joblib_resample_A_given_W(self, data):
        ...
        """
        if len(data) == 0:
            self.A = (np.random.rand(self.K, self.K) < self.network.P) * self.Amask
            return
        ...
        compute_weighted_impulses_at_events(S, C, Z, dt_max,
                                            min_supp, max_supp,
                                            W, mu, tau, lambda_ir)
    ...
    def resample_W_given_A_and_z(self, data):
        ...
        :return:
        """
        kappa_post, v_post = self._get_conditional_W_given_A_and_z(data)
        self.W = np.array(np.random.gamma(kappa_post, 1.0/v_post)).reshape((self.K, self.K))

    def _get_conditional_W_given_A_and_z(self, data):
