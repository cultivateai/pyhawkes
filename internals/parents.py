...
class ContinuousTimeParents(GibbsSampling):
    ...
    def update_suff_stats(self):
        ...
        ct_compute_suff_stats(S, C, Z, dt_max, min_supp, max_supp,
    ...
    def resample(self):
        ...
        # sample in cython
        ct_resample_Z_logistic_normal(
            S, C, Z, dt_max, min_supp, max_supp,
            lambda0, W, mu, tau)
    ...
    def compute_gamma_imp_suff_stats(self):
        # 0: count
        # 1: sum of the data
        # 2: product of the data
        S, C, Z, dt_max = self.S, self.C, self.Z, self.dt_max
        gam_imp_ss = np.zeros((3, self.K, self.K))
        for n in xrange(self.N):
            par = Z[n]
            if par == -1:
                continue
            else:
                gam_imp_ss[0, C[par], C[n]] += 1
